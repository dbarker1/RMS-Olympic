...
problem.parameters['Np'] = Np
problem.parameters['Ta'] = Ta
problem.parameters['m'] = m
problem.parameters['theta'] = theta
problem.parameters['phi'] = phi
problem.parameters['phicos'] = np.cos(phi)
problem.parameters['phisin'] = np.sin(phi)
...
problem.add_equation("  rho_ref*( dt(u) - (4/3)*dx(dx(u)) - dz(uz) - (1/3)*dx(wz) - (Ta**(1/2))*( v*phicos )) + dx(p) \
                        -dz_rho_ref*( uz + dx(w) )\
                        = -rho_ref*( u*dx(u) + w*uz ) ")
                        
# y-component of the momentum equation
problem.add_equation("  rho_ref*( dt(v) - dx(dx(v)) - dz(vz) - (Ta**(1/2))*( (w*phisin) - (u*phicos ) ) )  \
                        - dz_rho_ref*(vz) \
                        = -rho_ref*( u *dx(v) + w*vz ) ")

# z-component of the momentum equation
problem.add_equation("  rho_ref*T_ref*( dt(w) - X*s - (4/3)*dz(wz) - dx(dx(w)) - (1/3)*dx(uz) + (Ta**(1/2))*( v*phisin ) ) + T_ref*dz(p) + theta*m*p \
...
problem.add_equation("  T_ref*( Pr*dt(s) - dx(dx(s)) - dz(sz) ) + theta*(m+1)*sz \
                        ...
                        + 2*Y*( dx(u)*dx(u) + wz*wz + uz*dx(w) - (1/3)*(dx(u)+wz)*(dx(u)+wz) + (1/2)*(uz*uz + dx(w)*dx(w) + vz*vz + dx(v)*dx(v)) ) ")
...
analysis.add_task("integ( integ( sqrt(u*u + v*v + w*w) , 'x')/Lx, 'z')/Lz", layout='g', name='Re')
...
analysis.add_task("integ(0.5*rho_ref*(u*u + v*v + w*w)*w, 'x')*((Pr*Pr*theta)/Ra)/Lx", layout='g', name='L_KE')
...
analysis.add_task(" (integ( integ( 2*rho_ref*( dx(u)*dx(u) + wz*wz + uz*dx(w) - (1/3)*(dx(u)+wz)*(dx(u)+wz) + (1/2)*(uz*uz + dx(w)*dx(w) + vz*vz + dx(v)*dx(v)) ), 'x'), 'z')/(Lx*Lz) )*((Pr*Pr*theta)/Ra) ", layout='g', name='E_def')
