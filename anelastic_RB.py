...
import run_param_file2 as rpf   # Imports a parameter file "run_param_file2.py"        # Changed to run_param_file2 11/11/2019
...
problem.add_equation("  rho_ref*( dt(u) - (4/3)*dx(dx(u)) - dz(uz) - (1/3)*dx(wz) + (Ta**(1/2))*(sin_phi*u) ) - dz_rho_ref*(uz + dx(w)) + dx(p) \
             = - rho_ref*(u*dx(u) + w*uz ) ")
